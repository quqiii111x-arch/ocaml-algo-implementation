...
// EFFECT: Given a dictionary, return the pointer to the element(key-value pair)
// specified by the key, return NULL if not found.

void insert(void *dictionary, int key, int value);
// EFFECT: If the key does not exist, insert an element(a key-value pair) to
// this dictionary. If key already exists, update the value.

void delete(void *dictionary, int key);
// EFFECT: Delete an element(key-value pair) specified by the key. If it does
...
// EFFECT: retrieve the pointer to the element(key-value pair) just before a
// given key

void *successor(void *dictionary, int key);
// EFFECT: retrieve the pointer to the element(key-value pair) just after a
// given key
